...
def index():
    if request.method == "POST":
            try:
                ...
                kevaluator = Kami(
                    [response['reference'], response['prediction']],
                ...
                # test if versustext activate
                versus_text = {"comparaison": []}
                if bool(response['vtOpt']):
                    versus_text = show_diff_color_html(normalize("NFKD", str(response['reference'])), normalize("NFKD", str(response['prediction'])))
                # add this to jsonify if something wrong
                # with show_diff_color_html() : **{"reference": reference, "prediction": prediction},
                return jsonify({
                                **serialize_scores(kevaluator.scores.board),
                                **versus_text
                                }), 200
            except:
                jsonify({}), 400
